function cd = cd_diagram(s,labels,alpha)
%
...
figure(1);
set(gcf,'Units','centimeter','position',[5 5 6.5 3])
fontsize = 7;
linewidth = 0.5;
clf
axis off
% axis([-0.4 1.3 40 140]);
axis xy 
% set(gca,'position',[0.08 0.3 0.77 1])
tics = repmat((0:(k-1))/(k-1), 3, 1);
line(tics(:), repmat([100, 105, 100], 1, k), 'LineWidth', linewidth, 'Color', 'k');
tics = repmat(((0:(k-2))/(k-1)) + 0.5/(k-1), 3, 1);
line(tics(:), repmat([100, 102.5, 100], 1, k-1), 'LineWidth', linewidth, 'Color', 'k');
line([0 0 0 cd/(k-1) cd/(k-1) cd/(k-1)], [127 123 125 125 123 127], 'LineWidth', linewidth, 'Color', 'k');
h = text(0.5*cd/(k-1), 130, strcat('CD=',num2str(cd)), 'FontSize', fontsize, 'LineWidth', linewidth, 'HorizontalAlignment', 'center', 'Color', 'k');

for i=1:k

   text((i-1)/(k-1), 110, num2str(k-i+1), 'FontSize', fontsize, 'LineWidth', linewidth, 'HorizontalAlignment', 'center', 'Color', 'k');
...
for i=1:ceil(k/2)
   ...
   line([(k-r(i))/(k-1) (k-r(i))/(k-1) 1.2], [100 100-5*(n+1)-10*i 100-5*(n+1)-10*i],  'LineWidth', linewidth, 'Color', 'k');
%    h = text(1.3, 100 - 5*(n+1)- 10*i + 5, num2str(r(i)), 'FontSize', fontsize,'LineWidth', linewidth,  'HorizontalAlignment', 'right', 'Color', 'k');

   text(1.25, 100 - 5*(n+1) - 10*i+5, labels{idx(i)}, 'FontSize', fontsize, 'LineWidth', linewidth, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'left', 'Color', 'k');
...
for i=ceil(k/2)+1:k
   ...
   line([(k-r(i))/(k-1) (k-r(i))/(k-1) -0.2], [100 100-5*(n+1)-10*(k-i+1) 100-5*(n+1)-10*(k-i+1)],'LineWidth', linewidth,  'Color', 'k');

%    text(-0.25, 100 - 5*(n+1) -10*(k-i+1)+5, num2str(r(i)), 'FontSize', fontsize,'LineWidth', linewidth,  'HorizontalAlignment', 'left', 'Color', 'k');

   text(-0.25, 100 - 5*(n+1) -10*(k-i+1)+5, labels{idx(i)}, 'FontSize', fontsize, 'LineWidth', linewidth, 'VerticalAlignment', 'middle', 'HorizontalAlignment', 'right', 'Color', 'k');
...
for i=1:size(clique,1)
   ...
   line([((k-min(R))/(k-1)) + 0.015 ((k - max(R))/(k-1)) - 0.015], [100-5*i 100-5*i], 'LineWidth', 1, 'Color', 'k');
